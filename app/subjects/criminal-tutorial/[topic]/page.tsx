...
export default async function TopicPage({ params }: Params) {
  ...
  const subject = await db.query.subjects.findFirst({
    where: eq(subjects.slug, "criminal-tutorial"),
  });
  if (!subject) notFound();
  ...
  const base = `/subjects/criminal-tutorial/${topicSlug}/practice`;
    ...
    <div className="space-y-6">
      <header className="space-y-1">
        <p className="text-sm text-muted-foreground">
          ...
          /{" "}
          <Link href="/subjects/criminal-tutorial" className="hover:underline">
            刑法課輔
          </Link>{" "}
          / {topic.nameZh}
        </p>
